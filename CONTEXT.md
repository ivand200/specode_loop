...
**Workflow Kit**:
The checked-in Docker Sandbox mixin that delivers service-owned workflow files to each Sandbox Iteration after one successful runner preflight. The canonical kit is `sandbox-kits/workflow-skills`, and provisioning it never changes Target Project files.
_Avoid_: Copied project skill, host skill source, generated kit

**Service Implementation Skill**:
The `specode-loop-implement` skill stored completely inside the canonical Workflow Kit and explicitly requested by every Sandbox Iteration. A deliberately same-named Target Project skill keeps normal project-level precedence.
_Avoid_: Project `$do-work`, host workflow, shared skill
